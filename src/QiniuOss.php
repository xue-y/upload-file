...
class QiniuOss extends Base
{
    ...
    public function exceUpload(){
        ...
        $token = $auth->uploadToken($bucket,$saveName,3600,$policy);
        // 构建 UploadManager 对象
        ...
        if ($err !== null) {
            ...
            $data['file_root_path']=$data['file_url']=$config['remoteHost'] . '/' . $ret['key'];
    ...
    public function delFile(){
        ...
        $accessKey = $config['accessKey'];
        $secretKey = $config['secretKey'];
        $delImgName =str_replace($config['remoteHost'],'',$file_path);
        // 初始化签权对象
        $auth = new Auth($accessKey,$secretKey);
        //  管理资源
        $bucketManager = new BucketManager($auth,$config);
        // 删除文件操作
        $is_del = $bucketManager -> delete($config["bucket"],$delImgName);
        if($is_del){
